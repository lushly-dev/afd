...
export interface LocalStore {
	...
	updateTodo: (
		id: string,
		updates: Partial<Pick<Todo, 'title' | 'description' | 'priority' | 'completed'>>
	) => void;
...
export function useLocalStore(): LocalStore {
	...
	const createTodo = useCallback(
		(title: string, options?: { description?: string; priority?: Priority }) => {
			const newTodo: Todo = {
				id: generateId(),
				title,
	...
	const updateTodoId = useCallback((oldId: string, newId: string) => {
		...
		const current = getTodos();
		const updated = current.map((todo) => (todo.id === oldId ? { ...todo, id: newId } : todo));
		setTodos(updated);
