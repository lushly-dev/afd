...
## JTBD Testing Scenarios

### Scenario 1: Variable Resolution

```yaml
# scenarios/pipeline-variable-resolution.scenario.yaml
scenario:
  ...
  - name: "Execute multi-step pipeline"
    ...
    input:
      steps:
        ...
        - { command: "data-validate", input: { data: "$prev" } }  # 0.80
    expect:
      success: true
...
```json
// fixtures/basic-user.json
{
  "app": "afd-test",
  "clearFirst": true,
  "users": [
    { "id": 1, "name": "Alice", "tier": "standard" }
  ]
}
```
