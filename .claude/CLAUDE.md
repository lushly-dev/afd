...
Lefthook and `pnpm check` do **not** cover Python (`python/`), Rust (`packages/rust/`) or alfred. Each has its own path-filtered workflow, listed below. The Python and Rust todo example backends are not in any gate yet.
