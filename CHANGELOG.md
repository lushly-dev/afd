...
- **Python:** `create_retry_middleware()` backs off exponentially with a cap and jitter, like TypeScript, instead of linearly: retry `n` waits `min(max_delay, retry_delay * 2 ** (n - 1))` ms, randomized to between half and all of that. The new `max_delay` (default 5000) and `jitter` (default `True`) keyword arguments control it. Invalid options (a negative or non-integer `max_retries`, a negative `retry_delay` or `max_delay`, an infinite `max_delay`) now raise `ValueError`, as in TypeScript.
- **Python:** `ReconnectingHandoffConnection` passes the session to its reconnect command as `sessionId`, the wire name TypeScript uses, instead of `session_id`. The `ReconnectionOptions.session_id` attribute is unchanged; a reconnect command that read `session_id` must read `sessionId`.
...
- **Output Shape Predictability** (`@lushly-dev/afd-server`, `@lushly-dev/afd-testing`) — Optional `output` Zod schema on `defineCommand()` declares command output shape for agent introspection. Populates `returns` field and MCP `_meta.outputSchema`. New `missing-output-schema` surface validation rule (info severity).
