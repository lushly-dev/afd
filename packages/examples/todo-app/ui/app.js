...
function showToast(result, commandName) {
  ...
  if (result.confidence !== undefined) {
    ...
    const confidenceClass = result.confidence >= 0.9 ? '' : result.confidence >= 0.7 ? 'medium' : 'low';
    confidenceHtml = `
...
async function callTool(name, args = {}, options = {}) {
  ...
  const isMutation = ['todo.create', 'todo.update', 'todo.toggle', 'todo.delete', 'todo.clear'].includes(name);
  ...
  try {
    ...
    if (shouldLog) {
      if (result.success) {
        ...
        const time = result.metadata?.executionTimeMs !== undefined 
...
async function init() {
  ...
  setInterval(async () => {
    ...
    if (isConnected) {
      ...
      await Promise.all([loadTodos({ silent: true }), loadStats({ silent: true })]);
