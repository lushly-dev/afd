...
const previousTodosMap = new Map(); // Track todos for remote change detection
...
const _deleteSelectedBtn = document.getElementById('deleteSelected');
const _toggleSelectedBtn = document.getElementById('toggleSelected');
...
function showToast(result, commandName) {
	...
	if (result.confidence !== undefined) {
		...
		const confidenceClass =
			result.confidence >= 0.9 ? '' : result.confidence >= 0.7 ? 'medium' : 'low';
		confidenceHtml = `
	...
	if (result.warnings?.length > 0) {
		const warningItems = result.warnings
			.map((w) => `<div class="toast-warning-item">⚠️ ${escapeHtml(w.message)}</div>`)
			.join('');
...
async function loadTodos(options = {}) {
	...
	const result = await callTool('todo-list', { ...filterParams, limit: 100 }, { silent });
