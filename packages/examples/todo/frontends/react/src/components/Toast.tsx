...
const Toast: React.FC<ToastProps> = ({ toast, onRemove }) => {
  ...
  const confidencePercent = toast.confidence !== undefined 
    ? Math.round(toast.confidence * 100) 
    : null;
  
  const confidenceClass = toast.confidence !== undefined
    ? toast.confidence >= 0.9 ? "" : toast.confidence >= 0.7 ? "medium" : "low"
    : "";

  return (
    <div className={`toast ${toast.type} ${toast.isRemote ? "remote-change" : ""}`}>
      <span className="toast-icon">{toast.icon || (toast.type === "success" ? "✓" : toast.type === "error" ? "✗" : "⚠️")}</span>
      <div className="toast-content">
        ...
        <div className="toast-meta">
          ...
          {toast.executionTimeMs !== undefined && (
            <span>⚡ {toast.executionTimeMs}ms</span>
          )}
          {toast.isRemote && (
        ...
        {toast.warnings && toast.warnings.length > 0 && (
          <div className="toast-warnings">
            {toast.warnings.map((w, i) => (
              <div key={i} className="toast-warning-item">⚠️ {w.message}</div>
            ))}
          </div>
        )}
      </div>
...
export const useToast = () => {
  ...
  const showResultToast = useCallback((result: CommandResult<unknown>, commandName: string) => {
    addToast({
  ...
  const showRemoteChanges = useCallback((changes: RemoteChanges) => {
    ...
    if (changes.completed.length > 0) {
      ...
      notifications.push({
        type: "success",
        icon: "✅",
        message: count === 1 ? `Completed: ${titles}` : `${count} items completed: ${titles}${extra}`,
        isRemote: true,
      });
    }
    ...
    if (changes.uncompleted.length > 0) {
      ...
      notifications.push({
        type: "info",
        icon: "🔄",
        message: count === 1 ? `Reopened: ${titles}` : `${count} items reopened: ${titles}${extra}`,
        isRemote: true,
      });
    }
