...
async function main() {
  ...
  if (avgOverall < 1) {
    ...
    console.log(`${colors.green}✓ Expected speedup vs MCP: ${Math.round(30 / avgOverall)}x - ${Math.round(100 / avgOverall)}x${colors.reset}`);
  } else {
    console.log(`${colors.yellow}⚠ Direct transport is slower than expected (${avgOverall.toFixed(3)}ms)${colors.reset}`);
  }
