...
import type { ScenarioResult, StepResult, TestReport, TestSummary } from '../types/report.js';
...
export class TerminalReporter {
	...
	reportStepProgress(step: Step, result: StepResult, stepIndex: number, totalSteps: number): void {
		...
		this.write(
			`  ${icon} ${colorize(stepNum, 'dim', this.config.colors)} ${step.command} ${colorize(duration, 'gray', this.config.colors)}`
		);
		...
		if (this.config.verbose && result.assertions.length > 0) {
			for (const assertion of result.assertions) {
				...
				this.write(
					`      ${colorize(assertIcon, assertColor, this.config.colors)} ${assertion.description || assertion.path}`
				);
			}
		}
		...
		if (result.outcome === 'fail' || result.outcome === 'error') {
			if (result.error) {
				this.write(
					`      ${colorize(`Error: ${result.error.message}`, 'red', this.config.colors)}`
				);
			}
			...
			if (!this.config.verbose) {
				for (const assertion of result.assertions.filter((a) => !a.passed)) {
					this.write(
						`      ${colorize('✗', 'red', this.config.colors)} ${assertion.description || assertion.path}`
					);
				}
			}
		}
	...
	private reportScenarioHuman(result: ScenarioResult): void {
		...
		this.write('');
		this.write(
			`${icon} ${colorize(result.jobName, 'bold', this.config.colors)} ${colorize(`(${duration})`, 'dim', this.config.colors)}`
		);
		...
		if (this.config.verbose) {
			...
			for (const stepResult of result.stepResults) {
				...
				this.write(
					`  ${stepIcon} ${stepResult.command} ${colorize(stepDuration, 'gray', this.config.colors)}`
				);
		...
		if (!this.config.verbose && result.failedSteps > 0) {
			...
			for (const stepResult of result.stepResults.filter(
	...
	private reportSummaryHuman(results: ScenarioResult[]): void {
		...
		const failed = results.filter(
			(r) => r.outcome === 'fail' || r.outcome === 'error' || r.outcome === 'partial'
		).length;
		...
		this.write(
			`  ${colorize(`${passed} passed`, passColor, this.config.colors)}, ${colorize(`${failed} failed`, failColor, this.config.colors)}`
		);
		this.write(
			`  ${colorize(`Total time: ${this.formatDuration(totalDuration)}`, 'dim', this.config.colors)}`
		);
