...
import { calculateSummary, createEmptySummary } from '../types/report.js';
 ...
 */
function createErrorResult(path: string, jobName: string, _errorMessage: string): ScenarioResult {
	return {
		scenarioPath: path,
		jobName,
...
export async function scenarioEvaluate(
	...
	try {
		...
		const scenariosToRun: Array<{ scenario: Scenario; path: string }> = [];
...
export function formatTerminal(report: TestReport): string {
	...
	lines.push('━'.repeat(60));
	lines.push(
		`Scenarios: ${summary.totalScenarios} total, ${summary.passedScenarios} passed, ${summary.failedScenarios} failed`
	);
...
export function formatJunit(report: TestReport): string {
	...
	lines.push(
		`<testsuites name="JTBD Scenarios" tests="${summary.totalScenarios}" failures="${summary.failedScenarios}" time="${(report.durationMs / 1000).toFixed(3)}">`
	);
	...
	for (const [job, jobResults] of byJob) {
		...
		lines.push(
			`  <testsuite name="${escapeXml(job)}" tests="${jobResults.length}" failures="${jobFailed}" time="${(jobTime / 1000).toFixed(3)}">`
		);

		for (const result of jobResults) {
			const name = result.scenarioPath.split('/').pop() ?? 'unknown';
			lines.push(
				`    <testcase name="${escapeXml(name)}" time="${(result.durationMs / 1000).toFixed(3)}">`
			);
