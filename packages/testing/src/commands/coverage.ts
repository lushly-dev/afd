...
export async function scenarioCoverage(
  ...
  try {
    ...
    let scenariosToAnalyze: Array<{ scenario: Scenario; path: string }> = [];
    ...
    for (const { scenario } of scenariosToAnalyze) {
      ...
      const jobCoverage = jobMap.get(job);
      if (jobCoverage) {
        const scenariosForJob = scenariosToAnalyze.filter(
          (s) => s.scenario.job === job
        );
        const totalJobSteps = scenariosForJob.reduce(
          (sum, s) => sum + (s.scenario.steps?.length ?? 0),
          0
        );
        jobCoverage.avgSteps = Math.round(
          totalJobSteps / scenariosForJob.length
        );
      }
    }
    ...
    if (input.knownCommands && input.knownCommands.length > 0) {
      ...
      summary.commands.untested = input.knownCommands.filter(
        (cmd) => !testedCommands.has(cmd)
      );
      summary.commands.coverage =
        (summary.commands.tested / summary.commands.known) * 100;
    }

    if (input.knownErrors && input.knownErrors.length > 0) {
      summary.errors.known = input.knownErrors.length;
      summary.errors.untested = input.knownErrors.filter(
        (err) => !testedErrors.has(err)
      );
      summary.errors.coverage =
        (summary.errors.tested / summary.errors.known) * 100;
    }
...
export function formatCoverageTerminal(output: ScenarioCoverageOutput): string {
  ...
  lines.push(`Commands tested: ${summary.commands.tested}${summary.commands.known ? ` / ${summary.commands.known}` : ''}`);
  ...
  for (const cmd of topCommands) {
    ...
    lines.push(`  ${cmd.command}: ${cmd.stepCount} steps in ${cmd.scenarioCount} scenarios${errorFlag}`);
  }
  lines.push('');
...
export function formatCoverageMarkdown(output: ScenarioCoverageOutput): string {
  ...
  for (const cmd of commandCoverage) {
    lines.push(`| \`${cmd.command}\` | ${cmd.scenarioCount} | ${cmd.stepCount} | ${cmd.hasErrorTests ? '✅' : '❌'} |`);
  }
  lines.push('');
