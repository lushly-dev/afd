...
import { scenarioCreate, listTemplates } from '../commands/create.js';
...
const mockHandler: CommandHandler = async (command, input) => {
  switch (command) {
    case 'todo.create':
      return success({ id: '1', title: (input as any).title });
    case 'todo.list':
      return success({ items: [] });
    case 'todo.get':
      if ((input as any).id === 'non-existent') {
        return failure({ code: 'NOT_FOUND', message: 'Todo not found' });
      }
      return success({ id: (input as any).id, title: 'Test' });
    default:
...
describe('scenario.create', () => {
  ...
  it('creates error-handling template', async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.data?.scenario.steps.every(
      s => s.expect.success === false
    )).toBe(true);
  });
